...
def run(init_lr=0.1,
    ...
    label_id = set()
    count_dict = dict()
    #target = open('D:\Project\WLASL\code\I3D\preprocess\wlasl_100_top_1.txt','w')
    ...
    for data in dataloaders["test"]:
        ...
        out_labels = run_on_tensor(inputs)
        ...
        if out_labels[-1] == labels[0].item():
            correct += 1
            top1_tp[labels[0].item()] += 1
            if labels[0].item() not in label_id:
                label_id.add(labels[0].item())
                count_dict[labels[0].item()] = [0,0]
                
            count_dict[labels[0].item()][1] = count_dict[labels[0].item()][1] + 1
            count_dict[labels[0].item()][0] = count_dict[labels[0].item()][0] + 1
                
                #target.write('%s\t%s\t%s\n' % (labels[0].item(), wlasl_dict[labels[0].item()], video_id[0]))
        else:
            if labels[0].item() not in count_dict:
                count_dict[labels[0].item()] = [0,0]
            count_dict[labels[0].item()][0] = count_dict[labels[0].item()][0] + 1
...
if __name__ == '__main__':
    ...
    ensemble( mode = mode, root = root, train_split = train_split, weights = weights, num_classes= num_classes)
